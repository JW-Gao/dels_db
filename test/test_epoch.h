...
// 定义 LOG_INFO 宏
#define LOG_INFO(fmt, ...) \
    std::cout << "INFO: " << std::format(fmt, ##__VA_ARGS__) << std::endl;
...
private:
  ...
  static test_item_t *node_allocate(CepochImpl &epoch, bool refresh, std::atomic<test_item_t *> *free, uint64_t start,
    ...
    for (auto i = 0; i < test_items; ++i) {
      ...
      if (nullptr == ptr)
        continue;
      // 为什么item会有epoch，不是只有线程才会有epoch吗
...
public:
  ...
  static void concurrent_test() {
    ...
    for (auto i = 0; i < test_items; ++i)
      items.emplace_back((rnd() % 4 > 2) ? new test_item_t : nullptr); // 75% chance to be nullptr.
    ...
    std::cout << "Finish within " << (duration / 1000.0f) << "s. Now safe epoch: "
