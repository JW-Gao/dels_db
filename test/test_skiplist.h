...
// namespace zFast {
  ...
  #define TCS_RESERVED __COUNTER__
  // zrk
  struct reclaim_t final {

  };

  // get_tc 中生成的thread_context_t是thread_local的
  struct thread_context_t final {
    struct tcs_slots_t {
      tcs_t inner_slots[128];
      tcs_slots_t() {
        for (int i = 0; i < 128; ++i) {
          inner_slots[i] = 0;
        }
      }
      tcs_t & operator[](int32_t idx) {
        if (idx == -1) {
          for (int i = 0; i < 128; i ++) {
            if (inner_slots[i] == 0) {
              // inner_slots[i] = TCS_RESERVED;
              return inner_slots[i];
            }
          }
        }
        return inner_slots[idx];
      }
    };

    // int inner_tcs = 
    tcs_slots_t slots; // thread local slots, for epoch reclaim
    reclaim_t reclaim;
    Crandom rnd;
...
private:
  static thread_context_t *get_tc() {
    ...
    // 定义了一个线程本地静态原始字节缓冲区，大小和对齐方式等于 thread_context_t
  ...
  private:
    ...
    template <RegionType type> inline void prefetch_for_read_locality(node_t * node) const {
      dbg_assert(node != nullptr && node != head<type>());
      PREFETCH(node, 0, 1);
    }
  };
  ...
  static void concurrent_exchange_test() {
    ...
    for (auto tid = 0; tid < thread_number; ++tid) {
      threads.emplace_back([&epoch, &csl, tid, &exit]() {
        ...
        // TCS_RESERVED 是递增的，每次该宏出现的时候值就会加一
        epoch.register_epoch_thread(get_tc()->slots[TCS_RESERVED]);
    ...
    std::cout << "Finish within " << (duration.count() / 1000.0f) << "s. Now safe epoch: "
              << epoch.get_safe_reclaim_epoch(true) << std::endl;
    epoch.unregister_epoch_thread(get_tc()->slots[TCS_RESERVED]);
  }
};
